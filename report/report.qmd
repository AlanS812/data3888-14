...
```{r fig-appendix-pca, fig.cap="Cumulative variance against number of components plot to analyse PCA.", message=FALSE, warning=FALSE}
# install.packages(c("reticulate","ggplot2"))  # if not already installed
library(reticulate)
library(ggplot2)

# 1. Load joblib and your PCA model
joblib <- import("joblib")
pca    <- joblib$load("../app/Base_pca.joblib")

# 2. Extract explained variance ratios and compute cumulative sum
var_ratios <- pca$explained_variance_ratio_
cumvar     <- cumsum(var_ratios)

# 3. Prepare a data frame for plotting
df <- data.frame(
  Component = seq_along(cumvar),
  CumulativeVariance = cumvar
)

# 4. Plot
ggplot(df, aes(x = Component, y = CumulativeVariance)) +
  geom_line(color = "steelblue", size = 1) +
  geom_point(color = "darkred", size = 2) +
  # only show every 50th component on the x-axis
  scale_x_continuous(breaks = seq(0, max(df$Component), by = 50)) +
  labs(
